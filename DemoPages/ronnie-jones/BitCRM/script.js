document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('pullFromPearlDiver').addEventListener('click', function() {
        fetchDataFromPearlDiver();
    });
...
function initializeEmptyTable() {
    ...
    placeholderRow.innerHTML = `
        <td colspan="6" class="empty-table-message">
            No contacts loaded. Click "Pull From Pearl Diver" to load contact data.
        </td>
    `;
    tbody.appendChild(placeholderRow);
}

// Function to fetch data from the PearlDiver API
function fetchDataFromPearlDiver() {
    const button = document.getElementById('pullFromPearlDiver');
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            button.innerHTML = 'Pull From Pearl Diver';
        })
        .catch(error => {
            console.error('Error fetching data:', error);
            alert('Error fetching data from Pearl Diver. Please try again.');
...
function populateTableWithData(data) {
    ...
    contacts.forEach(contact => {
        ...
        row.innerHTML = `
            ...
            <td>${contact.email || ''}</td>
            <td>${contact.phone || ''}</td>
            <td>${contact.vehicleMake || 'N/A'}</td>
            <td>${contact.vehicleModel || 'N/A'}</td>
            <td>${contact.vehicleYear || 'N/A'}</td>
        `;
...
function showContactDetails(contact) {
    ...
    modalContent.querySelector('#transferToCDP').addEventListener('click', function() {
        ...
        const cdpData = [{
            ...
            "Vehicle Make": contact.vehicleMake || "",
            "Vehicle Model": contact.vehicleModel || "",
            "Vehicle Type": contact.vehicleType || "",
...
function showContactForm(contact = null) {
    ...
    if (isEdit) {
        ...
        // For new contacts, just provide basic fields
        formHtml += `
            <div class="form-group">
    ...
    modalContent.querySelector('#contactForm').addEventListener('submit', function(e) {
        ...
        if (isEdit) {
            ...
            // For new contacts, just get the basic fields
            formData.id = Date.now(); // Generate a unique ID
            formData.firstName = document.getElementById('firstName').value;
            formData.lastName = document.getElementById('lastName').value;
            formData.email = document.getElementById('email').value;
            formData.phone = document.getElementById('phone').value;
            formData.vehicleMake = document.getElementById('vehicleMake').value;
            formData.vehicleModel = document.getElementById('vehicleModel').value;
            formData.vehicleYear = document.getElementById('vehicleYear').value;
